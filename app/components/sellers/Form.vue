...
const state = ref<Seller>({ ...props.dataForm })
...
const title = computed(() => (props.action === 'create' ? 'New Seller' : 'Edit Seller'))
...
// Error messages
const getFieldErrors = (field: keyof Seller) =>
...
const processForm = async () => {
  ...
  try {
    ...
    if( props.action === 'create' ) {
      const { data } = await createSeller(state.value)
      actionProcess = data
    } else if ( props.action === 'edit' ) {
      const dataId = state.value._id
      const { data:updatedData } = await updateSeller(dataId!, state.value)
      actionProcess = updatedData
      await refreshNuxtData([`seller-${dataId}`])
    }
    ...
    await refreshNuxtData(['seller-list'])
          ...
          <v-row>
            ...
            <v-col cols="12" sm="3">
              <v-text-field
                ...
                @input="v$.fee.$touch()"
                @blur="v$.fee.$touch()"
                :error-messages="getFieldErrors('fee')"
              />
            </v-col>
