...
const state = ref<Package>({ ...props.dataForm })
...
const title = computed(() => (props.action === 'create' ? 'New Package' : 'Edit Package'))
...
// Error messages
const getFieldErrors = (field: keyof Package) =>
...
const processForm = async () => {
  ...
  try {
    ...
    if( props.action === 'create' ) {
      const { data } = await createPackage(state.value)
      actionProcess = data
    } else if ( props.action === 'edit' ) {
      const dataId = state.value._id
      const { data:updatedData } = await updatePackage(dataId!, state.value)
      actionProcess = updatedData
      await refreshNuxtData([`package-${dataId}`])
    }
    ...
    await refreshNuxtData(['package-list'])
          ...
          <v-row>
            <v-col cols="12" sm="8">
              <v-text-field density="compact"
                :label="t('trkgNum')"
                v-model="state.trkgNum"
                @input="v$.trkgNum.$touch()"
                @blur="v$.trkgNum.$touch()"
                :error-messages="getFieldErrors('trkgNum')"
              />
            </v-col>
