...
const title = ref('packageByCRList')
...
const { getPackagesByCR } = usePackage()
...
const viewItem = async (item:any) => {
  router.push({ path: `/packages/cr/${ item._id }` })
}
...
const loadItems = async (options:any) => {
  ...
  const { packagesByCR } = await getPackagesByCR(itemId.value, query)
  tableData.value = {
    items: packagesByCR?.items ?? [],
    total: packagesByCR?.total ?? 0
  }
